...
function init$rc() {
  ...
  $rc.util.extends=_extends;

  // Class hierarchy
  $rc.CombObject=function() {
  };
  ...
  $rc.CombObject.prototype.canonicalise=function() {
    // does nothing, override in Serial and parallel 
  }
  ...
  $rc.Serial.prototype={
    ...
    canonicalise : function() {
      ...
      this.vals.forEach(function(e) { 
        e.canonicalise();
        if(e instanceof $rc.Serial) {
          e.vals.forEach(function(f) {
            newVals.push(f);
          });
        }
        else {
  ...
  function shouldIncludeSeries(combType) {
    if(typeof(combType)=="undefined") {
      combType="B";
    }
    combType=(""+combType).toUpperCase();
    var r=("P"!=combType);
    return r;
  }
  
  function shouldIncludeParallel(combType) {
    if(typeof(combType)=="undefined") {
      combType="B";
    }
    combType=(""+combType).toUpperCase();
  ...
  function makeLBCombs(
    ...
    if(1==numComps) {
      ...
      // Parallel combinations then series combinations
        if(shouldIncludeParallel(combType)) {
        ...
        for(
          ...
          makeLBCombs(
            leftCompCount, 2*limit, 
            tolerance, srcVals, leftBranchConsumer,
            combType
          );
        }
      }
      ...
      // serial combinations
      if(shouldIncludeSeries(combType)) {
        ...
        for(var leftSegCount=1; leftSegCount<numComps; leftSegCount++) {
          ...
          makeLBCombs(
            leftSegCount, limit/2, tolerance, 
            srcVals, leftSegConsumer,
            combType
          );
        }
      }
    ...
    if(1==numComps) {
      ...
      if(shouldIncludeParallel(combType)) {
        for(var leftCompCount=1; leftCompCount<numComps; leftCompCount++) {
          ...
          makeLBCombs(
            leftCompCount, 2*limit, tolerance, 
            srcVals, leftBranchConsumer,
            combType
          );
        }
      }
  ...
  function makeVals(
    ...
    if(targetValue>0) {
      if(numComps==1) {
        ...
        makeVals(numComps-1, targetValue, tol, srcVals, notifyMe, combType);
